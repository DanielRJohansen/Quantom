...
steps = [5000,10000,15000,20000,25000,30000,35000,40000,45000,50000,55000,60000,65000,70000,75000,80000,85000,90000,95000];
rmsd = [2.44066,3.02694,2.57103,2.46585,2.67038,3.22128,2.64486,2.33483,0.2596,2.41551,2.904,2.53032,2.50013,2.60636,3.07592,2.60409,2.31353,0.457098,2.33518];
...
steps = [0, 5000,10000,15000,20000,25000,30000,35000,40000,45000,50000,55000,60000,65000,70000,75000,80000,85000,90000,95000, 100000];
length(steps)
rmsd = [ 5.075626177131198e-6 2.836169958114624 3.885155200958252 4.882663726806641 5.851119041442871 6.803428649902344 7.670094013214111 8.359801292419434 9.010626792907715 9.708115577697754 10.332955360412598 11.001035690307617 11.590282440185547 12.338804244995117 12.729470252990723 13.645112037658691 14.366199493408203 14.852843284606934 15.775565147399902 15.849276542663574 16.019229888916016 ];
plot(steps, rmsd, LineWidth=1.5)
xlabel("Time [ns]")
ylabel("RMSD [nm]")
%%
...
total_p_upperbound = 12997;
compound_p_upperbound = 46*56;
stride = total_p_upperbound
d = size(data)
force = reshape(data, [total_p_upperbound, steps]);
%force = force(compound_p_upperbound:end, :);    % water only
...
workdir = "C:\\PROJECTS\\Quantom\\Simulation\\Steps_3402";
file = fopen(strcat(workdir, "\\sim_traindata.bin"), "rb");
data = fread(file, 'single');
fclose(file);


p_per_compound = 56;
stride = p_per_compound*46*9;   % p in comp, total comp. floats/p
block = 35;
thread = 2;
x = data(7 + thread*9 + block*p_per_compound*9:stride:end);
y = data(8 + thread*9 + block*p_per_compound*9:stride:end);
z = data(9 + thread*9 + block*p_per_compound*9:stride:end);
%x=log(x);
%y=log(y);
%z=log(z);
length(x)
...
for i = 1:len+1
    ...
    bond_len(i) = abs(pos(2)-pos(1));

end

%tiledlayout(2,1)
%nexttile
plot(bond_len, LineWidth=1);
xlim([700, inf])
title("Bond len, 1D")
ylabel("Bond len [nm]")
xlabel("Step #")
